...
-- ────────────────────────────────────────
-- approve_pending_report — Approve Laporan Pending (SPV)
-- (UI saat ini memakai submit_report_direct status Approved; pertahankan untuk alur pending jika diaktifkan lagi)
-- ────────────────────────────────────────
CREATE OR REPLACE FUNCTION public.approve_pending_report(
  p_report_id UUID, p_approved_qty NUMERIC, p_notes TEXT DEFAULT NULL
)
RETURNS JSON
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public
AS $$
DECLARE
  v_report RECORD; v_item RECORD; v_spv_id UUID; v_final_stock NUMERIC; v_source TEXT;
BEGIN
  v_spv_id := auth.uid();
  ...
  IF v_final_stock < 0 THEN
    RAISE EXCEPTION 'Insufficient stock. Current: %, deduct: %.', v_item.stock, p_approved_qty;
  END IF;

  v_source := CASE WHEN v_report.type = 'Usage' THEN 'REPORT_USAGE' ELSE 'REPORT_DAMAGE' END;

  UPDATE public.trx_reports
  SET status = 'Approved', quantity = p_approved_qty, notes = COALESCE(p_notes, notes),
      reviewed_by = v_spv_id, reviewed_at = NOW()
  ...
  INSERT INTO public.trx_stock_log (
    item_id, report_id, changed_by, change_amount, previous_stock, final_stock, source, notes
  ) VALUES (v_item.id, p_report_id, v_spv_id, -p_approved_qty, v_item.stock, v_final_stock, v_source, p_notes);

  RETURN json_build_object(
    'success', true, 'report_id', p_report_id, 'item_id', v_item.id,
    'old_stock', v_item.stock, 'new_stock', v_final_stock, 'approved_qty', p_approved_qty
  );
END;
$$;
